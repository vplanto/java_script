---
title: "Лекція 6: Архітектура Інтернету. OSI, TCP/IP та DNS"
type: lecture
module: 4
prerequisites:
  - "05_json_data"
...
Слайди зібрані в одному PDF поруч із матеріалом лекції, щоб можна було відкрити їх з того ж місця, що й конспект: [Internet_Architecture_Blueprint.pdf](assets/Internet_Architecture_Blueprint.pdf).
...
DNS — це розподілена база даних, яка перетворює `facebook.com` -> `157.240.214.35`. Протокол — **[16] RFC 1035** ([реєстр джерел](sources.md)).
