...
У прикладі нижче вже є **`try/catch`**, перевірка `response.ok` і повідомлення про помилки (у т.ч. неіснуюче місто → 404).
...
Логіка роботи з мережею побудована на асинхронних запитах:
1. **`fetch(url)`** — ініціює асинхронний мережевий HTTP-запит до сервера.
2. **`await fetch(url)`** — призупиняє виконання коду, поки від сервера не прийдуть перші заголовки відповіді та статус-код. Цей крок повертає об'єкт `Response`.
3. **`response.ok`** — перевіряє статус-код (чи знаходиться він у діапазоні 200–299). Якщо ні (наприклад, 404), ми вручну кидаємо помилку `throw new Error(...)`.
4. **`await response.json()`** — асинхронно дозавантажує решту тіла відповіді (stream) та десеріалізує її з формату JSON у звичайний JavaScript-об'єкт.
5. **`try/catch/finally`** — перехоплює винятки: як суто мережеві помилки (наприклад, обрив зв'язку, коли `fetch` сам викидає виняток), так і помилкові HTTP-статуси (які потрапляють у `catch` лише тому, що ми попередньо перевірили `response.ok` і вручну згенерували виняток через `throw new Error`). Блок `finally` гарантовано повертає інтерфейс до початкового стану (вимикає індикатор завантаження).
...
> [!NOTE]
> **Зверніть увагу на конструкцію `w?.icon` та `wind?.speed`:**
> Символи `?.` — це оператор **опціонального ланцюжка** (optional chaining). Він дозволяє безпечно зчитувати властивості об'єкта, навіть якщо сам об'єкт дорівнює `null` або `undefined`. Якщо `w` чи `wind` відсутні в отриманих даних, код не завершиться помилкою `TypeError`, а просто поверне `undefined` (або дефолтне значення завдяки оператору `??`).

> [!WARNING]
> **Передача ключів доступу (API keys/Tokens) в URL-параметрах (Query Parameters) — це погана практика (Bad Practice):**
> * **Чому це небезпечно:** Повні URL-адреси запитів (разом із параметрами на кшталт `?appid=...`) майже завжди записуються у відкритому вигляді в логи вебсерверів, проксі-серверів, балансувальників навантаження або CDN. Крім того, вони зберігаються в історії браузера та можуть передаватися стороннім сайтам у заголовку `Referer`. Авторизаційні дані безпечніше передавати в HTTP-заголовках (наприклад, `Authorization: Bearer <token>`), які зазвичай не логуються.
> * **Чому OpenWeatherMap робить саме так:** 
>   1. **Простота використання:** Це дозволяє розробникам-початківцям протестувати запит прямо в адресному рядку браузера або за допомогою простої команди `curl` без налаштування додаткових HTTP-заголовків.
>   2. **Уникнення CORS Preflight:** Використання кастомних HTTP-заголовків авторизації (наприклад, `X-Api-Key`) змушує браузер спочатку виконувати додатковий асинхронний запит `OPTIONS` (CORS Preflight), щоб перевірити дозволи. Передача токена в URL для простого `GET`-запиту дозволяє уникнути цього зайвого кроку, що зменшує мережеву затримку (latency) та навантаження на сервери API.
