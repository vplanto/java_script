...
1. Чому ігровий цикл використовує `requestAnimationFrame(loop)`, а не `setInterval(loop, 16)`?

<details>
<summary>Відповідь</summary>

`requestAnimationFrame` синхронізується з монітором (60, 120, 144 Hz) і авто-пауза у прихованих вкладках. `setInterval(16)` може запускатись частіше або рідше за реальний refresh rate, накопичує затримки і не зупиняється, коли вкладка в фоні — марна трата CPU.

</details>

2. Поясніть алгоритм AABB. Чому він називається «Axis-Aligned Bounding Box»?

<details>
<summary>Відповідь</summary>

AABB перевіряє перетин двох прямокутників, sides яких паралельні осям координат (вирівняні вздовж Axis). Умова: по X — правий бік A > лівого B і лівий A < правого B; аналогічно по Y. Він простий і дуже швидкий, але не підходить для повернутих об'єктів.

</details>

3. В чому помилка коду `player.y = event.clientY` для керування ракеткою набору Pong?

<details>
<summary>Відповідь</summary>

`clientY` — координата відносно вікна браузера. Canvas може знаходитись не у лівому верхньому куті. Потрібно відняти зміщення: `player.y = event.clientY - canvas.getBoundingClientRect().top`.

</details>

4. Чим «God Mode» AI відрізняється від «Predictive» AI? Який з них реалістичнішe виглядає для гравця?

<details>
<summary>Відповідь</summary>

God Mode телепортує ракетку до м'яча щокадру — неможливо обіграти, але виглядає неприродно. Predictive AI прораховує траєкторію і рухається до точки зустрічі з певною швидкістю — програє на неочікуваних рикошетах, але виглядає як «справжній» противник.
