...
3. **Resolution:** Агент `A` помирає, якщо існує ворог `E`, що його атакує, такий що:
3. **Resolution:** Агент `A` помирає, якщо існує ворог `E`, що його атакує, такий що: `EnemiesTargeting(A) > EnemiesTargeting(E)`.

   Оскільки всі індивідуальні агенти базово є рівними за силою, для знищення ворога необхідна сувора чисельна перевага.

> **Наслідок 1 (Focus Fire):** 2 агенти, що разом атакують одного ворожого агента, вбивають його і виживають. Одинак завжди програє групі.
> **Наслідок 2 (Stalemate):** При зіткненні 1 на 1 сили рівні (`EnemiesTargeting(A) == EnemiesTargeting(E)`), тому жоден з агентів не гине. Вони просто блокують один одного.
